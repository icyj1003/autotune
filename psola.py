...
def find_peaks(signal, fs, max_hz=950, min_hz=75, analysis_win_ms=40, max_change=1.005, min_change=0.995):
    ...
    # analysis sequence length in samples
    sequence = int(analysis_win_ms / 1000 * fs)
    periods = compute_periods_per_sequence(
        signal, sequence, min_period, max_period)
...
def psola(signal, peaks, f_ratio):
    ...
    for j in range(len(new_peaks)):
        ...
        if peaks[i] + P1[1] > N - 1:
            P1[1] = N - 1 - peaks[i]
        # linear OLA window
        window = list(np.linspace(
            0, 1, P1[0] + 1)[1:]) + list(np.linspace(1, 0, P1[1] + 1)[1:])
        # center window from original signal at the new peak
        new_signal[new_peaks[j] - P1[0]: new_peaks[j] + P1[1]
                   ] += window * signal[peaks[i] - P1[0]: peaks[i] + P1[1]]
    return new_signal
